    ...
    <div>
      ...
      {uploadPct === null ? (
        ...
        >
          ...
          <span className="block mt-2 text-[12px] text-white/70 font-light">
        ...
        <div className="bio-cell w-full px-7 py-9">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="font-mono text-[12px] text-white/75">{uploadPct}%</span>
          </div>
          ...
          <p className="mt-3 text-[12px] text-white/65 font-light">
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        {loading && (
          <p className="text-[13px] text-white/65 font-light col-span-full">Cargando tu galería…</p>
        )}
        {!loading && videos.length === 0 && (
          <p className="text-[13px] text-white/65 font-light col-span-full">
        ...
        {videos.map((v) => (
          ...
          >
            <div className="relative" style={{ aspectRatio: "16/9" }}>
              {v.status === "ready" ? (
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-2">
                  ...
                  <span className="font-mono text-[9px] tracking-[0.2em] uppercase text-white/75">
            ...
            <div className="flex items-center justify-between px-4 py-3">
              <span className="text-[13px] text-white/85 font-light truncate pr-3">
                {v.title}
              </span>
              <button
                type="button"
                onClick={() => onDelete(v)}
                className="font-mono text-[9px] tracking-[0.15em] uppercase text-white/60 hover:text-red-400/90 transition-colors bg-transparent border-0 cursor-pointer flex-shrink-0"
              >
                Borrar
              </button>
            </div>
      ...
      {playing && (
        ...
        >
          <div className="w-full max-w-[960px]" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <span className="text-[14px] text-white/85 font-light truncate pr-4">{playing.title}</span>
              <button
                type="button"
                onClick={() => setPlaying(null)}
